...
class SeedTDGCompiler(BaseTDGCompiler):
    ...
    def compile_tests(self, context: str, query: str) -> str:
        ...
        prompt = (
            ...
            "=== DESIGN RULE ===\n"
            "- Prefer checklist-style tests: one rubric requirement per test function.\n"
            "- Keep each test focused on exactly one failure signal with a clear message.\n\n"
            ...
            "   Build a reusable forbidden_terms list and check with one helper pattern:\n"
            "       forbidden_terms = ['term_a', 'term_b']\n"
            "       assert not any(t in answer.lower() for t in forbidden_terms)\n"
